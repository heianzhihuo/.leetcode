...
class Solution {
    public List<List<String>> suggestedProducts(String[] products, String searchWord) {
        // 思路比较简单，第一步products排序
        // 对应searchWord[i]时，找出对应始末位置a,b
        // i+1时，a和b都从上一次位置开始搜索，
        //时间复杂度nlogn,其中n=products.length
        Arrays.sort(products);//排序
        List<List<String>> ret = new ArrayList<>();
        int a = 0,b = products.length-1;
        for(int i=0;i<searchWord.length();i++){
            char ch = searchWord.charAt(i);
            for(;a<=b;a++){
                if(i<products[a].length() && products[a].charAt(i)==ch) break;
            }
            for(;a<=b;b--){
                if(i<products[b].length() && products[b].charAt(i)==ch) break;
            }
            List<String> tmp = new ArrayList<>();
            for(int j=0;j<3 && j+a<=b;j++){
                tmp.add(products[a+j]);
            }
            ret.add(tmp);
        }
        return ret;
